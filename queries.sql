...
WHERE genres.name = 'Science Fiction & Fantasy' AND
...
SELECT movies.title, movies.year, genres.name AS genre FROM actors
JOIN cast_members ON actors.id = cast_members.actor_id
JOIN movies ON cast_members.movie_id = movies.id
JOIN genres ON movies.genre_id = genres.id
...
SELECT movies.title, movies.year, studios.name AS studio FROM movies
JOIN genres ON movies.genre_id = genres.id
